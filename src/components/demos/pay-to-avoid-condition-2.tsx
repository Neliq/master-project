...
export function PayToAvoidCond2({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      title="Pay To Avoid: Restorative Monetization"
      caption="Restorative Monetization — each step requires exponentially more effort." auditorStats={stats}>
        ...
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          ...
          <div className="space-y-0.5">
            {Array.from({length: level}, (_, i) => (
              <div key={i} className="bg-slate-600 h-2 rounded-sm" style={{width: (100 - i * 12) + "%"}} />
            ))}
          </div>
        </div>
        <button onClick={() => setLevel(l => l + 1)} className="bg-slate-600 hover:bg-slate-700 text-white w-full rounded-md py-2 text-xs font-medium">
