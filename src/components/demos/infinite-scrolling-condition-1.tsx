...
export function InfiniteScrollingCond1({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Autonomous Content Injection — price inflates step-by-step as fees are injected." auditorStats={stats}>
      <div className="space-y-3">
        ...
        {step < 3 ? (
          <button onClick={() => setStep(s => s + 1)} className="bg-amber-500 hover:bg-amber-600 text-white w-full rounded-md py-2 text-xs font-medium">
            Reveal next fee
          </button>
        ) : (
