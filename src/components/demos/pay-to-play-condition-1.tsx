...
export function PayToPlayCond1({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Exponential Friction and Paid Bypass — each step requires exponentially more effort." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          ...
          <div className="bg-foreground/10 h-3 rounded-full overflow-hidden">
            <div className="bg-pink-500 h-full transition-all" style={{width: Math.min(level * 15, 100) + "%"}} />
          </div>
        </div>
