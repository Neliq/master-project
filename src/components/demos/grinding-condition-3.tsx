...
export function GrindingCond3({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      title="Grinding: Diminishing Yields over Time"
      caption="Diminishing Yields over Time — each step requires exponentially more effort." auditorStats={stats}>
        ...
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          ...
          <div className="space-y-0.5">
            {Array.from({length: level}, (_, i) => (
              <div key={i} className="bg-red-500 h-2 rounded-sm" style={{width: (100 - i * 12) + "%"}} />
            ))}
          </div>
        </div>
