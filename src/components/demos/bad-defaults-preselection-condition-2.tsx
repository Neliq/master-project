...
export function BadDefaultsPreselectionCond2({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Semantic Intent of the Default Action — privacy-sensitive options pre-checked, exploiting inertia." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          <div className="mb-2 font-medium">Semantic Intent of the Default Action — Preferences</div>
          <div className="space-y-2">
            {items.map((item, i) => (
              <label key={i} className="flex items-center justify-between border-t py-2">
                <span className="text-[11px]">{item.label}</span>
                <span className={`text-[10px] font-medium ${item.checked ? "text-purple-700 dark:text-purple-300" : "text-muted-foreground"}`}>
                  {item.checked ? "ON" : "OFF"}
                </span>
              </label>
            ))}
