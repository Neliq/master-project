...
export function WatchAdsToUnlockFeaturesCond1({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Attention as Transactional Currency — content is blocked without payment." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          <div className="mb-1 font-medium">Attention as Transactional Currency</div>
          <div className="rounded-md border-pink-500/40 bg-pink-500/5 p-4 text-center" style={{opacity: unlocked ? 1 : 0.2}}>
            <div className="text-2xl">{unlocked ? "🔓" : "🏦"}</div>
            <div className="mt-1 text-[10px]">{unlocked ? "Vault opened" : "Premium vault"}</div>
          </div>
          {!unlocked && (
            <button onClick={() => setUnlocked(true)} className="bg-pink-500 hover:bg-pink-600 text-white mt-2 w-full rounded-md py-2 text-[10px] font-medium">
              Open vault — $19.99/mo
            </button>
          )}
        </div>
