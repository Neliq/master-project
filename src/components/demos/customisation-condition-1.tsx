...
export function CustomisationCond1({
  ...
  const [acceptedVia, setAcceptedVia] = React.useState<"layer1" | "custom">("layer1");

  const reset = () => {
    setCustomising(false);
    setOnIds(CATEGORIES.filter((c) => c.on).map((c) => c.id));
    setOutcome("none");
    setAcceptedVia("layer1");
  };
          ...
          {outcome !== "none" && (
            <div className="rounded-md border border-green-500/30 bg-green-500/5 p-2.5 text-[9px] leading-relaxed">
              <div className="flex items-center gap-1.5 font-semibold text-green-700 dark:text-green-300 uppercase tracking-tight">
                ...
                {outcome === "accepted" ? "Consent granted (1 click)" : "Tracking rejected (1 click)"}
              </div>
              <p className="mt-0.5 text-muted-foreground">
                {outcome === "accepted"
                  ? "Accepting cost one click. Rejecting would also have cost one click: d(S_0, S_reject_all) = 1, so no asymmetry exists."
                  : "Rejecting cost exactly one click, identical to accepting. The path of least resistance is not biased toward the provider."}
              </p>
            </div>
          )}
        </div>
