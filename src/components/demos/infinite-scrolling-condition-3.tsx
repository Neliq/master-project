...
export function InfiniteScrollingCond3({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Absence of State Anchoring — there is no way to dismiss this element." auditorStats={stats}>
      <div className="space-y-3">
        ...
        <div className="border-purple-500/40 bg-purple-500/5 rounded-r-md border-l-4 p-4 text-xs">
          <div className="font-medium text-purple-700 dark:text-purple-300">Absence of State Anchoring</div>
          <p className="text-muted-foreground text-[10px] mt-1">Panel with no close mechanism.</p>
          <button onClick={() => setDone(true)} className="bg-purple-500 hover:bg-purple-600 text-white mt-2 rounded-md px-3 py-1.5 text-[10px] font-medium">
            Acknowledge
          </button>
        </div>
      </div>
