...
export function IntermediateCurrencyCond2({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Lexical Tokenization mapped to Forced Exchange — price inflates step-by-step as fees are injected." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          <div className="mb-2 font-medium">Lexical Tokenization mapped to Forced Exchange</div>
        ...
        {step < 3 ? (
          <button onClick={() => setStep(s => s + 1)} className="bg-orange-500 hover:bg-orange-600 text-white w-full rounded-md py-2 text-xs font-medium">
            Reveal next fee
          </button>
        ) : (
