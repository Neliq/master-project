...
export function BundlingCond2({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      ...
      caption="Irreversible Set Addition — price inflates step-by-step as fees are injected." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          <div className="mb-2 font-medium">Irreversible Set Addition</div>
          <div className="flex items-end gap-1 h-16 mb-2">
            {fees.map((f, i) => (
              <div key={i} className="flex-1 flex flex-col items-center">
                <div className="bg-indigo-500 w-full rounded-t-sm" style={{height: (f.amount / total * 60) + "px"}} />
        ...
        {step < fees.length - 1 ? (
          <button onClick={() => setStep(s => s + 1)} className="bg-indigo-500 hover:bg-indigo-600 text-white w-full rounded-md py-2 text-xs font-medium">Continue</button>
        ) : (
