...
export function CustomisationCond3({
  ...
  const [onIds, setOnIds] = React.useState<string[]>(["d1", "d2", "d3"]);
  ...
  const renderDarkToggle = (label: string, id: string) => (
  ...
  const renderBenignToggle = (label: string, note: string, id: string) => (
    <label key={id} className="flex cursor-pointer items-start gap-2 rounded-md border border-border bg-background p-2 transition-colors hover:border-foreground/20">
      <input
        type="checkbox"
        checked={onIds.includes(id)}
        ...
        <div className="space-y-3">
          <div className="rounded-md border bg-card p-3">
            ...
            <div className="space-y-1.5">
              {BENIGN_LABELS.map((label, i) => renderBenignToggle(label, BENIGN_NOTES[i], `b${i}`))}
            </div>
            <button
          ...
          {saved && (
            <div className="rounded-md border border-green-500/30 bg-green-500/5 p-2.5 text-[9px] leading-relaxed">
              ...
              <p className="mt-0.5 text-muted-foreground">
                Every label is plain language with an explanation attached — jargon ratio{" "}
                {BENIGN_RATIO.toFixed(2)} &le; &tau;_jargon = {TAU_JARGON.toFixed(2)}. You knew exactly what
        ...
        <div className="rounded-md border bg-card p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="font-mono text-[8px] tabular-nums text-muted-foreground/50">
              jargon ratio {DARK_RATIO.toFixed(2)}
            </span>
          </div>
          <div className="space-y-1.5">
            {DARK_LABELS.map((label, i) => renderDarkToggle(label, `d${i}`))}
          </div>
          <button
            onClick={() => setSaved(true)}
          ...
          >
            {translated ? "Hide plain-language translations" : "Show plain-language translations"}
          </button>
        </div>
        ...
        {mode === "auditor" && saved && (
          <div className="rounded-md border border-yellow-500/30 bg-yellow-500/5 p-2.5 text-[9px] leading-relaxed space-y-1.5">
            ...
            <p className="text-muted-foreground">
              ...
              without external knowledge. {onIds.length} of the {DARK_LABELS.length} toggles remain on.
            </p>
          </div>
        )}
      </div>
