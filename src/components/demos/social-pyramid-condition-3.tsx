...
export function SocialPyramidCond3({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      title="Social Pyramid: Asymmetric Value Exchange"
      caption="Asymmetric Value Exchange — one action is trivially easy, the other is disproportionately hard." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border p-3 text-xs">
          <div className="mb-2 font-medium">Asymmetric Value Exchange</div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <div className="bg-slate-600 hover:bg-slate-700 text-white rounded-md px-3 py-1.5 text-[10px]">1-click Accept</div>
              <div className="bg-foreground/10 h-2 w-full rounded-full">
                <div className="bg-slate-600 h-2 rounded-full" style={{width: "100%"}} />
              </div>
            </div>
            <div className="flex items-center gap-2">
