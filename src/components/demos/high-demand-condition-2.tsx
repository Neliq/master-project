...
export function HighDemandCond2({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      title="High Demand: Strategic Temporal Injection"
      caption="Strategic Temporal Injection — price inflates step-by-step as fees are injected." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          <div className="mb-2 font-medium">Strategic Temporal Injection</div>
        ...
        {step < 3 ? (
          <button onClick={() => setStep(s => s + 1)} className="bg-teal-500 hover:bg-teal-600 text-white w-full rounded-md py-2 text-xs font-medium">
            Reveal next fee
          </button>
        ) : (
