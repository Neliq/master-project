...
export function EndorsementAndTestimonialsCond3({
  ...
  return (
    <DemoShell mode={mode} annotations={annotations} onRestart={onRestart ?? reset}
      title="Endorsement And Testimonials: Coordinated Temporal Injection"
      caption="Coordinated Temporal Injection — price inflates step-by-step as fees are injected." auditorStats={stats}>
      <div className="space-y-3">
        <div className="rounded-md border bg-foreground/5 p-3 text-xs">
          <div className="mb-2 font-medium">Coordinated Temporal Injection — Checkout</div>
          {shown.map((f, i) => (
        ...
        {step < fees.length - 1 ? (
          <button onClick={() => setStep(s => s + 1)} className="bg-pink-500 hover:bg-pink-600 text-white w-full rounded-md py-2 text-xs font-medium">
            Continue (more fees)
          </button>
        ) : (
