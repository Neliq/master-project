...
## Condition 1: 1. Exponential Effort Scaling

Original formula (verbatim): `E(L_i \to L_{i+1}) \propto c^i \quad (c > 1) \quad \land \quad V(L_{i+1}) \approx V(L_i) + k`
...
Original formula (verbatim): `\frac{\Delta P_{k}}{\Delta P_{1}} < e^{-\lambda(k-1)} \quad \land \quad \frac{d^2 P}{di^2} < 0`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
