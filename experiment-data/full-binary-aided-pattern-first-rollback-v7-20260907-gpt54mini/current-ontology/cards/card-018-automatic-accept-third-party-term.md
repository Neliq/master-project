...
## Condition 1: 1. Bundled Transitive Consent

Original formula (verbatim): `\mathrm{Accept}(T_{\mathrm{primary}}) = \mathrm{True} \quad \land \quad \forall t_i \in T_{\mathrm{third\_party}} : \mathrm{Accept}(t_i) = \mathrm{True} \quad \land \quad \nexists \mathrm{Toggle}(t_i) \in \mathrm{DOM}`
...
## Condition 2: 2. Opaque Entity Resolution

Original formula (verbatim): `E_{\mathrm{declared}} \subset E_{\mathrm{actual}} \quad \land \quad \frac{|E_{\mathrm{actual}}|}{\max(1,|E_{\mathrm{declared}}|)} > \tau_{\mathrm{entity\_skew}}`
Operational reading: **Evaluate the source formula over directly observed evidence; preserve every Boolean operator, quantifier, comparison, unit, scope, and threshold.**
Polarity: `trigger`
...
## Condition 3: 3. Semantic Concealment of Third-Party Agreement Language

Original formula (verbatim): `T(N_{\mathrm{legal}}) \cap K_{\mathrm{agreement}} \neq \emptyset \quad \land \quad \mathrm{Checked}(N_{\mathrm{legal}}) = \mathrm{True} \quad \land \quad \mathrm{UserToggled}(N_{\mathrm{legal}}) = \mathrm{False}`
Operational reading: **Evaluate the source formula over directly observed evidence; preserve every Boolean operator, quantifier, comparison, unit, scope, and threshold.**
Polarity: `trigger`
Comparison policy: Use the original formula's explicit threshold or equality; do not invent values.
Review policy: TRUE requires a complete observed proof; FALSE requires a false atom or direct counterevidence; UNKNOWN is retained when a required atom, threshold, scope, or observation is unavailable. Do not recode UNKNOWN as BENIGN without a declared mapping.
