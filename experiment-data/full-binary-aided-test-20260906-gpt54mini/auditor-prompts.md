...
Process every task in `/home/neliq/Coding/master-project/experiment-data/full-binary-aided-test-20260906-gpt54mini/assignments/not-aided/agent-XX.json` exactly once and in order. Write exactly one JSON object per task to `/home/neliq/Coding/master-project/experiment-data/full-binary-aided-test-20260906-gpt54mini/results/raw-not-aided/agent-XX.jsonl`.
...
Process every task in `/home/neliq/Coding/master-project/experiment-data/full-binary-aided-test-20260906-gpt54mini/assignments/aided/agent-XX.json` exactly once and in order. Write exactly one JSON object per task to `/home/neliq/Coding/master-project/experiment-data/full-binary-aided-test-20260906-gpt54mini/results/raw-aided/agent-XX.jsonl`.

Use a real browser. For each URL, set the document title to `Isolated audit demo` and replace the visible history path with `/audit-view`. Inspect only the rendered page and controls inside `[data-isolated-demo]`. Do not read source code, `score-only/`, other assignments, or prior results. Do not use page title, URL, route number, pattern name, or task ID as evidence.

The only aid is `/home/neliq/Coding/master-project/experiment-data/full-binary-aided-test-20260906-gpt54mini/arms/aided/formalizations-pseudocode.md`. It contains general pseudocode rules without demonstrations or labelled cases. First make `base_label` from live evidence. Then nominate at most three plausible rule IDs from the page and apply only those candidates; do not apply all rules globally.
