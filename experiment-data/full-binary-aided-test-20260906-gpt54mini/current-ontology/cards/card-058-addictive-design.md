...
Original formula (verbatim): `Y_{\mathrm{max}}(t) - Y_{\mathrm{scroll}}(t) < \tau_{\mathrm{buffer}} \quad \land \quad Y_{\mathrm{max}}(t + \Delta t) > Y_{\mathrm{max}}(t) \quad \land \quad E_{\mathrm{append}}(t) = \mathrm{True}`
...
Original formula (verbatim): `\begin{gathered} \bigl[\forall e \in E_{\mathrm{stop}} : \mathrm{Visible}(e, t) = \mathrm{False} \quad \lor \quad \frac{A(e)}{A_{\mathrm{viewport}}} < \tau_{\mathrm{min\_visible}} \\ \lor \quad \mathrm{CR}(e, L_{\mathrm{bg}}) < 3.0\bigr] \quad \land \\ T_{\mathrm{session}} > \tau_{\mathrm{hyper\_engagement}} \end{gathered}`
...
Original formula (verbatim): `\frac{|\{w \in T : w \in L_{\mathrm{reinforcement}}\}|}{A_{\mathrm{viewport}}} > \tau_{\mathrm{reinforcement\_density}}`
