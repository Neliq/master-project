...
## Condition 1: 1. Manufactured Livelihood Dependency

Original formula (verbatim): `T_{\mathrm{fiat}} = 0 \quad \land \quad \mathrm{Asserted}(M_{\mathrm{pitch}},\mathrm{Failure}(I_{\mathrm{creator}})) = \mathrm{True}`
...
## Condition 2: 2. Visual Proximity of Anthropomorphic Imagery to Action Prompts

Original formula (verbatim): `\min_{i \in I_{\mathrm{face}}} d_{\mathrm{spatial}}(i, N_{\mathrm{prompt}}) < \tau_{\mathrm{social}} \quad \land \quad \frac{A(i)}{A_{\mathrm{viewport}}} > 0.05`
Operational reading: **Evaluate the source formula over directly observed evidence; preserve every Boolean operator, quantifier, comparison, unit, scope, and threshold.**
Polarity: `trigger`
...
## Condition 3: 3. Emotional Asymmetry

Original formula (verbatim): `E_{\mathrm{user}} = \mathrm{Refusal} \quad \land \quad \frac{\Delta V_{\mathrm{emotion}}(A_{\mathrm{mascot}})}{\Delta t} < -\tau_{\mathrm{emotional\_change}}`
Operational reading: **Evaluate the source formula over directly observed evidence; preserve every Boolean operator, quantifier, comparison, unit, scope, and threshold.**
Polarity: `trigger`
Comparison policy: A direct within-interface comparison may satisfy the qualitative relation when both operands and the direction are explicitly observed; no numeric threshold may be invented.
Review policy: TRUE requires a complete observed proof; FALSE requires a false atom or direct counterevidence; UNKNOWN is retained when a required atom, threshold, scope, or observation is unavailable. Do not recode UNKNOWN as BENIGN without a declared mapping.
