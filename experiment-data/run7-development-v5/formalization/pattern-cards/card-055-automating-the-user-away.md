...
## Condition 1: 1. Autonomous Action Execution

Original formula (verbatim): `A_{\mathrm{critical}} = \mathrm{Executed} \quad \text{given} \quad E_{\mathrm{user}} = \emptyset \quad \land \quad t \ge \tau_{\mathrm{system}}`
...
## Condition 2: 2. Omission of the Interrupt Vector

Original formula (verbatim): `B_{\mathrm{cancel}} \notin \mathrm{DOM}(t) \quad \lor \quad \Delta t_{\mathrm{warning}} < \tau_{\mathrm{reaction}}`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
...
## Condition 3: 3. Semantic Speed of Consent-Timing Language

Original formula (verbatim): `t_{\mathrm{window}} < 2.0 \quad \land \quad \mathrm{SemanticType}(T_{\mathrm{prompt}}) = \mathrm{TimedConsent}`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
