...
## Condition 1: 1. Interception of the Fiat Checkout Flow

Original formula (verbatim): `\mathrm{target}(E_{\mathrm{purchase}}) = V_{\mathrm{exchange}} \quad \land \quad \mathrm{target}(E_{\mathrm{purchase}}) \neq V_{\mathrm{checkout}}`
...
## Condition 2: 2. Visual Obscuration of Real-Currency Equivalence

Original formula (verbatim): `N_{\mathrm{real}} = \emptyset \quad \lor \quad \frac{\mathrm{fontSize}(N_{\mathrm{real}})}{S_{\mathrm{base}}} < 0.5`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
...
## Condition 3: 3. Lexical Tokenization mapped to Forced Exchange

Original formula (verbatim): `T_{\mathrm{price}}(n) \in C_{\mathrm{virtual}} \quad \land \quad \text{TransactionStatus}(n) \implies \text{Executed}(E_{\mathrm{exchange}})`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
