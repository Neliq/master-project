...
## Condition 1: 1. Structural Element Downgrading

Original formula (verbatim): `\mathrm{Tag}(B_{\mathrm{business}}) = \texttt{<button>} \quad \land \quad \mathrm{Tag}(B_{\mathrm{user}}) = \texttt{<a>} \quad \land \quad S_{\mathrm{padding}}(B_{\mathrm{user}}) \approx 0`
...
## Condition 2: 2. Relational Visual Weight Disparity

Original formula (verbatim): `W(x) = \alpha \cdot A(x) + \beta \cdot C(x) + \gamma \cdot F(x)`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
...
## Condition 3: 3. Strict Semantic Opposition

Original formula (verbatim): `\exists \mathrm{Intent}(L(B_{1})) \equiv \neg \mathrm{Intent}(L(B_{2}))`
Operational reading: **Strict observable evaluation of the original predicate**
Polarity: `trigger`
