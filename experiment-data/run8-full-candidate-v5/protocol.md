# Run 8 full candidate experiment — formalization revision v4

## Purpose

Run 8 tests the frozen `formalization-revision-v4` package against an evidence-only baseline on the complete 372-instance corpus. It follows the bounded C1 repair improvement but remains a new experiment; Run 6 and all development diagnostics remain unchanged.
...
11. Evaluate each pattern's three conditions as alternative detectors: `Pattern[p] = C[p,1] OR C[p,2] OR C[p,3]`, where each `C[p,j]` includes its formula and every GIVEN clause. TRUE requires any condition TRUE; FALSE requires all conditions FALSE; otherwise UNKNOWN.
